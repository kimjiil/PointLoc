...
data_path = r"E:\git_repo\PointLoc\dataset\syswin\2024_0403_1608_54".replace("\\", '/')

device = f"cuda:0" if torch.cuda.is_available() else "cpu"
model = PointLoc()
model_path = r"C:\Users\jikim\Desktop\working_temp\2024_0405_1705_14/last_model.pt".replace("\\", '/')
...
def update(val):
    ...
    ax1.plot(gt_amr_box[:, 0] + gt_translation[0], gt_amr_box[:, 1] + gt_translation[1], color='blue', label="실제 움직임")
    ax1.plot(pred_amr_box[:, 0] + pred_translation[0], pred_amr_box[:, 1] + pred_translation[1], color='red', label="딥러닝 모델 예측")
    ...
    plt.savefig(f"C:/Users/jikim/Desktop/test_save_fig/scan_frame_{str(val).rjust(4,'0')}.png")
    cv2.imwrite(f"C:/Users/jikim/Desktop/test_save_fig/camera_frame_{str(val).rjust(4,'0')}.png", img)
